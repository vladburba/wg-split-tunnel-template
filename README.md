...
Если у вас уже есть рабочий WireGuard-конфиг (например, full-tunnel) и вам **не хочется
ставить Python, qrencode, запускать скрипты** — просто скопируйте одну длинную строку.

**Шаг 1.** Откройте файл [`allowed_ips_oneline.txt`](allowed_ips_oneline.txt) в этом репозитории.
Там одна строка ~1600 символов через запятую — это уже агрегированный список префиксов:
`1.1.1.1/32` (DNS) + AS Telegram + AS Google/YouTube + AS Meta/WhatsApp.

**Шаг 2.** Скопируйте всё содержимое файла.

**Шаг 3.** У себя сделайте **копию** существующего рабочего конфига (например, `wg-split.conf`).
Важно: оригинал full-tunnel профиля не трогайте — он понадобится для сайтов, которые в
split-tunnel не работают (см. ниже).

**Шаг 4.** В копии в секции `[Peer]` найдите строку:

```ini
AllowedIPs = 0.0.0.0/0,::/0
```

или похожую. **Замените** её на:

```ini
AllowedIPs = <вставьте сюда содержимое allowed_ips_oneline.txt>
```

**Шаг 5.** В секции `[Interface]` убедитесь что есть `DNS = 1.1.1.1`. Если нет — добавьте.
Без этой строки DNS-запросы пойдут мимо туннеля, провайдер их перехватит и подменит — и
Telegram/YouTube перестанут открываться **по доменным именам**, даже когда все правильные
IP уже в списке.

**Шаг 6.** Импортируйте отредактированный `wg-split.conf` в WireGuard app как **новый**
профиль (iPhone: + → Создать из файла; Mac/Windows: Add Tunnel → Import; Android: то же
самое из файла или из QR). Старый full-tunnel профиль **оставьте** — будет нужен.

Всё. Telegram, YouTube, WhatsApp теперь ходят через VPN, остальной трафик — напрямую.

> Если хочется чтобы скрипт сделал всё сам (создал .conf, подставил AllowedIPs, сгенерировал
> QR) — листайте ниже до раздела **Quick start**.

## Чем отличается от других решений

Похожие проекты есть, но решают разные задачи. Что выбрать:
...
| **Свой WG-сервер уже есть и работает**, нужен **второй split-tunnel профиль** именно под TG/YT/WA — без перенастройки сервера | **Этот шаблон** |
